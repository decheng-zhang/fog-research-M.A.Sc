...
public class RankingASFs {
	...
	/**
	 * Ranking solutions in frontiers considering several ASFs (one by each vector
	 * of weights)
	 * 
	 * @param solutionSet
	 *            The <code>SolutionSet</code> to be ranked
	 * @param asf
	 *            The <code>AchievementScalarizingFunction</code> to be used
	 * @param weights
	 *            The weight vectors to be used for each
	 *            <code>AchievementScalarizingFunction</code>
	 * @param normalization
	 *            True if the ASF should be normalized
	 */
	public RankingASFs(SolutionSet solutionSet, AchievementScalarizingFunction asf, double[][] weights,
			boolean normalization) {
		this.solutionSet_ = solutionSet;

		// List that contains the index of a solution and the value of the ASF (one for
		// each vector of weights)
		...
		// The list l is used to allow delete a solution when it is assigned to a
		// frontier
		// Each node in the list contain the index of the solution in the SolutioSet
	 ...
	 *            The <code>SolutionSet</code> to be ranked
	 * @param asfInUtopian
	 *            The <code>AchievementScalarizingFunction</code> to be used in the
	 *            utopian point
	 * @param pairWeights
	 *            The weight vectors to be used for each
	 *            <code>AchievementScalarizingFunction</code> in the utopian point
	 * @param asfInNadir
	 *            The <code>AchievementScalarizingFunction</code> to be used in the
	 *            nadir point
	 * @param oddWeights
	 *            The weight vectors to be used for each
	 *            <code>AchievementScalarizingFunction</code> in the nadir point
	 * @param normalization
	 *            True if the ASF should be normalized
	 */
	public RankingASFs(SolutionSet solutionSet, AchievementScalarizingFunction asfInUtopian, double[][] pairWeights,
		...
		// List that contains the index of a solution and the value of the ASFs (one for
		// each vector of weights)
		LinkedList<Node> l = new LinkedList<Node>();
		LinkedList<Node> solutionsToRemove = new LinkedList<Node>();
		Node bestNode;
	...
	/**
	 * If a set of weight vectors was divided considering the pair and odd
	 * positions, this method can be used to combine both set of weight vectors to
	 * generate the original set of weight vectors.
	 * 
	 * @param pairweights
	 *            A set of pair weight vectors (in a previous set of weight vectors)
	 * @param oddWeights
	 *            A set of odd weight vectors (in a previous set of weight vectors)
	...
	private double[][] unionPairAndOddASFValues(double[][] pairweights, double[][] oddWeights) {
		...
		for (int indexValue = 0; indexValue < oddWeights.length; indexValue++) {
			...
			for (int componentIndex = 0; componentIndex < pairweights[indexValue].length
					+ oddWeights[indexValue].length; componentIndex++) {
