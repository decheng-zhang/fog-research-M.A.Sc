...
public class MetricsUtil {
	 ...
	 * @return double [] An array of noOjectives values whit the maximun values for
	 *         each objective
	 **/
	 ...
	 * @param noObjectives
	 *            Number of objectives in the pareto front
	 * @return double [] An array of noOjectives values whit the minimum values for
	 *         each objective
	 **/
	...
	/**
	 * This method returns the distance (taken the euclidean distance) between two
	 * points given as <code>double []</code>
	 * 
	 * @param a
	 *            A point
	 * @param b
	...
	/**
	 * Gets the distance between a point and the nearest one in a given front (the
	 * front is given as <code>double [][]</code>)
	 * 
	 * @param point
	 *            The point
	 * @param front
	...
	/**
	 * Gets the distance between a point and the nearest one in a given front, and
	 * this distance is greater than 0.0
	 * 
	 * @param point
	 *            The point
	 * @param front
	...
	/**
	 * Reads a set of non dominated solutions from a file and store it in a existing
	...
	/**
	 * Calculates the hv contribution of different populations. Receives an array of
	 * populations and computes the contribution to HV of the population consisting
	 * in the union of all of them
	 * 
	 * @param populations,
	...
	public double[] hvContributions(SolutionSet[] populations) {
		...
		for (int i = 0; i < populations.length; i++) {
			if (invertedFront[i + 1].length == 0)
				...
				if (invertedFront[i + 1].length != invertedFront[0].length) {
					...
					// System.out.println(hypervolume.calculateHypervolume(aux, aux.length,
		...
		// System.out.println(invertedFront[0].length +" "+ invertedFront[i+1].length +"
		// "+ contribution[i]);
	...
	public double[] hvContributions(SolutionSet archive, SolutionSet[] populations) {
		...
		for (int i = 0; i < populations.length; i++) {
			if (invertedFront[i + 1].length == 0)
				 ...
				 * 
				 * 
				 * double [][] aux = new double[size2 - invertedFront[i+1].length][]; int index
				 * = 0; for (int j = 0; j < populations.length; j++) { if (j!=i) { for (int k =
				 * 0; k < invertedFront[j+1].length; k++) aux[index++] = invertedFront[j+1][k];
				 * } }
				 * 
				 * System.out.println(hypervolume.calculateHypervolume(invertedFront[0],
				 * invertedFront[0].length, getNumberOfObjectives));
				 ...
				 * 
				 * 
				 * contribution[i] = hypervolume.calculateHypervolume(invertedFront[0],
				 * invertedFront[0].length, getNumberOfObjectives) -
