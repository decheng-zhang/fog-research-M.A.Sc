	...
	public synchronized void algorithmSettings(String problemName, int problemIndex, Algorithm[] algorithm)
		...
		try {
			...
			if ((Boolean) parameters[0].get("estimatePoints_")) {
				...
				double[] idealPoint = paretoFrontInformation.getMinimumValues(paretoFront,
						problem.getNumberOfObjectives());
				double[] nadirPoint = paretoFrontInformation.getMaximumValues(paretoFront,
						problem.getNumberOfObjectives());
				double[] utopianPoint = new double[idealPoint.length];
				for (int indexObjective = 0; indexObjective < idealPoint.length; indexObjective++) {
					double quantity = Math.abs((nadirPoint[indexObjective] - idealPoint[indexObjective])
							* (Double) parameters[0].get("utopianValueInPercent_")) / 100;
				...
				asfInUtopian = new AchievementScalarizingFunction(new ReferencePoint(utopianPoint), nadirPoint,
						idealPoint);
	...
	private static void generateCombinedObjectivesFiles(GWASFGAStudy3D exp) throws IOException {
		...
		for (int algorithm = 0; algorithm < exp.algorithmNameList_.length; algorithm++) {
			for (int problem = 0; problem < exp.problemList_.length; problem++) {
				...
				String plotFileName = new String(exp.experimentBaseDirectory_ + "/" + exp.algorithmNameList_[algorithm]
						+ "-" + exp.problemList_[problem] + ".bat");
				FileWriter plotFile = new FileWriter(plotFileName, true);

				String command = new String();
				if (numberOfObjectives == 2)
					command = "gnuplot -e \"plot '../" + exp.paretoFrontDirectory_ + "/" + exp.paretoFrontFile_[problem]
							+ "' with lines linewidth 3 linecolor 9 tit 'Pareto optimal front ("
							+ exp.problemList_[problem] + ")', '" + fileName.substring(fileName.indexOf("/") + 1)
							+ "' with points pointtype 6 pointsize 1 linecolor 1;pause mouse close\";";
				else if (numberOfObjectives == 3)
					command = "gnuplot -e \"splot '../" + exp.paretoFrontDirectory_ + "/"
							+ exp.paretoFrontFile_[problem]
							+ "' with points linewidth 1 linecolor 9 tit 'Pareto optimal front ("
							+ exp.problemList_[problem] + ")', '" + fileName.substring(fileName.indexOf("/") + 1)
							+ "' with points pointtype 6 pointsize 1 linecolor 1;pause mouse close\";";
