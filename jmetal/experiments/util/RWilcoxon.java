...
public class RWilcoxon {
	...
	public static void generateScripts(String[] problems, String prefix, Experiment experiment) throws IOException {
		...
		for (int indicator = 0; indicator < experiment.indicatorList_.length; indicator++) {
			...
			output = "latexHeader <- function() {" + "\n" + "  write(\"\\\\documentclass{article}\", \"" + texFile
					+ "\", append=TRUE)" + "\n" + "  write(\"\\\\title{StandardStudy}\", \"" + texFile
					+ "\", append=TRUE)" + "\n" + "  write(\"\\\\usepackage{amssymb}\", \"" + texFile
			...
			latexTableCaption = "  write(\"\\\\caption{\", \"" + texFile + "\", append=TRUE)" + "\n"
					...
					+ experiment.indicatorList_[indicator] + ".}\", \"" + texFile + "\", append=TRUE)" + "\n";
			...
			output = "latexTableHeader <- function(problem, tabularString, latexTableFirstLine) {" + "\n"
					...
					// " write(\"\\\\begin{tabular}{" + latexTabularAlignment + "}\", \"" + texFile
					// + "\", append=TRUE)" + "\n" +
			...
			output = "latexTail <- function() { " + "\n" + "  write(\"\\\\end{document}\", \"" + texFile
					+ "\", append=TRUE)" + "\n" + "}" + "\n";
			os.write(output + "\n");

			if ((Boolean) experiment.indicatorMinimize_.get(experiment.indicatorList_[indicator])) {// minimize by
																									// default
			...
			for (int i = 1; i < experiment.algorithmNameList_.length; i++) {
				...
				latexTableFirstLine += " & \\\\multicolumn{" + problems.length + "}{c|}{"
