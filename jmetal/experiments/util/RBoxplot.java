...
public class RBoxplot {
	 ...
	 * of the results previosly obtained. The boxplots will be arranged in a grid of
	 * rows x cols. As the number of problems in the experiment can be too high,
	...
	public static void generateScripts(int rows, int cols, String[] problems, String prefix, boolean notch,
		...
		for (int indicator = 0; indicator < experiment.indicatorList_.length; indicator++) {
			...
			os.write("par(mar=c(10,6,4,2)+0.1)\n");
			os.write("boxplot(");
			for (int i = 0; i < experiment.algorithmNameList_.length; i++) {
				os.write(experiment.algorithmNameList_[i] + ",");
			} // for
			if (notch) {
				os.write("names=algs, las=2 ,notch = TRUE)" + "\n");
			} else {
				os.write("names=algs, las=2 ,notch = FALSE)" + "\n");
			}
			os.write("titulo <-paste(indicator, \"" + experiment.experimentName_ + "\", sep=\":\")" + "\n");
			os.write("title(main=titulo)" + "\n");
			os.write("mtext(\" SELF DEFINE LINE\", side=1, line=6)\n");
			os.write("}" + "\n");
