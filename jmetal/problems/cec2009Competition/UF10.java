...
public class UF10 extends Problem {
	...
	/**
	 * Constructor. Creates a default instance of problem CEC2009_UF10 (30 decision
	 * variables)
	 * 
	 * @param solutionType
	...
	public void evaluate(Solution solution) throws JMException {
		...
		solution.setObjective(0,
				Math.cos(0.5 * Math.PI * x[0]) * Math.cos(0.5 * Math.PI * x[1]) + 2.0 * sum1 / (double) count1);
		solution.setObjective(1,
				Math.cos(0.5 * Math.PI * x[0]) * Math.sin(0.5 * Math.PI * x[1]) + 2.0 * sum2 / (double) count2);
