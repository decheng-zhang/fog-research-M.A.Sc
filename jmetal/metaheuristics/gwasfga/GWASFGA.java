...
import jmetal.util.RankingASFs;
import jmetal.util.Weights;
...
public class GWASFGA extends Algorithm {
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		// If true, the nadir and ideal point of the Achievement Scalarizing Functions
		...
		while (evaluations < maxEvaluations) {
			...
			// Ranking the union considering the values of the Achievement Scalarizing
			// Functions
			RankingASFs ranking = new RankingASFs(union, asfInUtopian, this.pairWeights_, asfInNadir, this.oddWeights_,
					normalization);
		...
		RankingASFs ranking = new RankingASFs(population, asfInUtopian, this.pairWeights_, asfInNadir, this.oddWeights_,
				normalization);
	...
	/**
	 * Update the ideal point in the two Achievement Scalarizing Functions (one for
	 * the ideal point and the other one for the nadir point) considering the values
	 * of the given <code>Solution</code> In fact, we use the utopian value (a value
	 * better than the ideal). So, the best values are improved
	 * 
	 * @param individual
	 */
	private void updateIdealPoint(Solution individual) {
		...
		for (int n = 0; n < problem_.getNumberOfObjectives(); n++) {
			if (individual.getObjective(n) <= this.asfInUtopian.getIdeal()[n]) {
				improvementValue = (this.utopianValueInPercent
						* (this.asfInUtopian.getNadir()[n] - individual.getObjective(n))) / 100;
	...
	/**
	 * Update the nadir point in the two Achievement Scalarizing Functions (one for
	 * the ideal point and the other one for the nadir point) considering the values
	 * of the given <code>Solution</code> In fact, we use a worse nadir value (a
	 * value worse than the ideal). So, the worst values are worsen
	 * 
	 * @param individual
	 */
	private void updateNadirPoint(Solution individual) {
		...
		for (int n = 0; n < problem_.getNumberOfObjectives(); n++) {
			if (individual.getObjective(n) >= this.asfInNadir.getNadir()[n]) {
				deteriorationValue = (this.utopianValueInPercent
						* (this.asfInUtopian.getIdeal()[n] - individual.getObjective(n))) / 100;
	 ...
	 * Update the nadir point in the two Achievement Scalarizing Functions (one for
	 * the ideal point and the other one for the nadir point) considering the values
	 * of the solutions in the given <code>SolutionSet</code> In fact, we use a
	 * worse nadir value (a value worse than the ideal). So, the worst values are
	 * worsen
	 * 
	 * @param population
	 */
	private void updateNadirPoint(SolutionSet population) {
		...
		for (int n = 0; n < problem_.getNumberOfObjectives(); n++) {
			...
			deteriorationValue = (this.utopianValueInPercent
					* (this.asfInUtopian.getIdeal()[n] - population.get(0).getObjective(n))) / 100;
			deteriorationValue = Math.abs(deteriorationValue);
