 ...
 *         Algorithm" Published in Journal of Global Optimization in 2014 DOI =
...
public class WASFGA_main {
	 ...
	 *             jmetal.metaheuristics.wasfga.WASFGA_main problemName
	 *             referencePointFile - jmetal.metaheuristics.wasfga.WASFGA_main
	 *             problemName referencePointFile paretoFrontFile
	 *
	 *             Usage if objectives number is greater than two: -
	 *             jmetal.metaheuristics.wasfga.WASFGA_main problemName
	 *             referencePointFile weightsDirectory -
	 *             jmetal.metaheuristics.wasfga.WASFGA_main problemName
	 *             referencePointFile paretoFrontFile weightsDirectory
	 * 
	 *             The referencePointFile is a text file containing, in one row, the
	 *             wished value of each objective, separated by spaces
	 * 
	 *             The weightsDirectory is a folder containing the weights vector
	 *             files. The name of these files are WND_P.txt, where N is the
	 *             number of objectives and P is the population size (one vector for
	 *             each solution)
	 */
	public static void main(String[] args) throws JMException, SecurityException, IOException, ClassNotFoundException {
		...
		if (args.length == 2) {
			...
			if (problem.getNumberOfObjectives() > 2) {
				logger_.severe(
						"If the number of objectives is greater than two, a weights vector folder must be specified. ");
				System.exit(-1);
			}
		} // if
		...
		if (referencePoint.size() != problem.getNumberOfObjectives()) {
			logger_.severe(
					"The number of components of the reference point must be equal to the number of objectives.");
			System.exit(-1);
		}
