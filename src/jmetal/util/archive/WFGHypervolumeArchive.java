...
public class WFGHypervolumeArchive extends Archive {
	 ...
	 */
	/*
	 * public void actualiseHVContribution() { if (size() > 2) { // The contribution
	 * can be updated double[][] frontValues = this.writeObjectivesToMatrix(); int
	 * getNumberOfObjectives = objectives_; // STEP 1. Obtain the maximum and
	 * minimum values of the Pareto front double[] maximumValues =
	 * utils_.getMaximumValues(this.writeObjectivesToMatrix(),
	 * getNumberOfObjectives); double[] minimumValues =
	 * utils_.getMinimumValues(this.writeObjectivesToMatrix(),
	 * getNumberOfObjectives); // STEP 2. Get the normalized front double[][]
	 * normalizedFront = utils_.getNormalizedFront(frontValues, maximumValues,
	 * minimumValues); // compute offsets for reference point in normalized space
	 * double[] offsets = new double[maximumValues.length]; for (int i = 0; i <
	 * maximumValues.length; i++) { offsets[i] = offset_ / (maximumValues[i] -
	 * minimumValues[i]); } // STEP 3. Inverse the pareto front. This is needed
	 * because the original //metric by Zitzler is for maximization problems
	 * double[][] invertedFront = utils_.invertedFront(normalizedFront); // shift
	 * away from origin, so that boundary points also get a contribution > 0 for
	 * (double[] point : invertedFront) { for (int i = 0; i < point.length; i++) {
	 * point[i] += offsets[i]; } }
	 * 
	 * // calculate contributions and sort double[] contributions =
	 * utils_.hvContributions(objectives_, invertedFront); for (int i = 0; i <
	 * contributions.length; i++) { // contribution values are used analogously to
	 * crowding distance this.get(i).setCrowdingDistance(contributions[i]); } } } //
	 ...
	 */
	/*
	 * public int getLocation(Solution solution) { int location = -1; int index = 0;
	 * while ((index < size()) && (location!=-1) ) { if (equals_.compare(solution,
	 * get(index))==0) { location = index; } index++; } return location; }
	 */
