...
public class Ranking {
	...
	public Ranking(SolutionSet solutionSet) {
		 ...
		 * solutionSet_.size(); p++) { // Initialice the list of individuals that i
		 * dominate and the number // of individuals that dominate me iDominate[p] = new
		 * LinkedList<Integer>(); dominateMe[p] = 0; // For all q individuals ,
		 * calculate if p dominates q or vice versa for (int q = 0; q <
		 * solutionSet_.size(); q++) { flagDominate
		 * =constraint_.compare(solutionSet.get(p),solutionSet.get(q)); if (flagDominate
		 * == 0) { flagDominate
		 ...
		 * 
		 * // If nobody dominates p, p belongs to the first front if (dominateMe[p] ==
		 * 0) { front[0].add(new Integer(p)); solutionSet.get(p).setRank(0); } }
		 */
