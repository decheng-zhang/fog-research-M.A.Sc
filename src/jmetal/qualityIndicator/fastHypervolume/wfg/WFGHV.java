		...
		if (currentDimension_ == 2)
			...
			for (int i = front.nPoints_ - 1; i >= 0; i--) {
				volume += Math.abs(front.getPoint(i).objectives_[currentDimension_]
						- referencePoint_.objectives_[currentDimension_]) * this.getExclusiveHV(front, i);
			}
			currentDimension_++;
		}
	...
	public void makeDominatedBit(Front front, int p) {
		...
		for (int i = 0; i < z; i++)
			for (int j = 0; j < currentDimension_; j++) {
				fs_[currentDeep_].getPoint(i).objectives_[j] = worse(front.points_[p].objectives_[j],
						front.points_[p + 1 + i].objectives_[j], false);
			}
