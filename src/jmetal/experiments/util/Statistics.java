...
public class Statistics {
	...
	/**
	 * Calculates the median of a vector considering the positions indicated by the
	...
	public static Double calculateMedian(Vector vector, int first, int last) {
		...
		if (size % 2 != 0) {
			...
			median = ((Double) vector.elementAt(first + size / 2 - 1) + (Double) vector.elementAt(first + size / 2))
					/ 2.0;
		}
	...
	public static Double calculateIQR(Vector vector) {
		...
		if (vector.size() > 1) { // == 1 implies IQR = 0
			if (vector.size() % 2 != 0) {
				...
				// ;
				// System.out.println("Q3: [" + (vector.size()/2+1) + ", " + (vector.size()-1) +
				// "]= " + q3) ;
			} else {
				...
				// System.out.println("Q1: [" + 0 + ", " + (vector.size()/2 - 1) + "] = " + q1)
				// ;
				// System.out.println("Q3: [" + (vector.size()/2) + ", " + (vector.size()-1) +
				// "]= " + q3) ;
			} // else
		} // if
