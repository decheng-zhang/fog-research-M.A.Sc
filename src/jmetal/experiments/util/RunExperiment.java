...
public class RunExperiment extends Thread {
	...
	public void run() {
		...
		while (!finished) {
			...
			if (!finished && problemData != null) {
				...
				File computationTime = new File(directory + "/ExecutionTime");
				File rIter = new File(directory + "/RequiredIter");
				if (!computationTime.exists()) {
					try {
						boolean result = new File(directory + "/ExecutionTime").createNewFile();
					} catch (IOException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
					System.out.println("Creating Execution Time File");
				}
				if (!rIter.exists()) {
					try {
						boolean result = new File(directory + "/RequiredIter").createNewFile();
					} catch (IOException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
					System.out.println("Creating requireIter File");
				}
				...
				try {
					try {
						...
						// System.out.println((int)algorithm[alg].getOutputParameter("evaluations"));
				...
				printExecutionTime(directory + "/ExecutionTime", estimatedTime);
				// printRequiredIteration(directory +
				// "/RequiredIter",(int)algorithm[alg].getOutputParameter("evaluations"));
	...
	public void printRequiredIteration(String path, int iter) {
		try {
	...
	public void printExecutionTime(String path, long time) {
		try {
			FileOutputStream fos = new FileOutputStream(path);
			OutputStreamWriter osw = new OutputStreamWriter(fos);
			BufferedWriter bw = new BufferedWriter(osw);
