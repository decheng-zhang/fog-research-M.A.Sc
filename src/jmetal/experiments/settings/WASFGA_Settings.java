...
/**
 * @author Rubén Saborido Infantes Settings class of algorithm WASF-GA (real
 *         encoding)
 */
public class WASFGA_Settings extends Settings {
	...
	public WASFGA_Settings(String problem) throws JMException {
		...
		if (problem_.getName().substring(0, problem_.getName().length() - 1).equals("ZDT")
				|| problem_.getName().substring(0, problem_.getName().length() - 1).equals("LZ09_F"))
			convergenceIndicator_ = new QualityIndicator(problem_, paretoFrontDirectory + problem_.getName() + ".pf");
		else
			convergenceIndicator_ = new QualityIndicator(problem_,
					paretoFrontDirectory + problem_.getName() + "." + problem_.getNumberOfObjectives() + "D.pf");
	} // WASFGA_Settings
