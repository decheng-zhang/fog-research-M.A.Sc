...
public class LZ09 {
	...
	void alphaFunction(double alpha[], Vector<Double> x, int dim, int type) {
		if (dim == 2) {
			...
			if (type == 32) {
				alpha[0] = 1 - Math.cos(x.elementAt(0) * Math.PI / 2) * Math.cos(x.elementAt(1) * Math.PI / 2);
				alpha[1] = 1 - Math.cos(x.elementAt(0) * Math.PI / 2) * Math.sin(x.elementAt(1) * Math.PI / 2);
			...
			if (type == 33) {
				...
				alpha[2] = 3 - (Math.sin(3 * Math.PI * x.elementAt(0)) + Math.sin(3 * Math.PI * x.elementAt(1)))
						- 2 * (x.elementAt(0) + x.elementAt(1));
			}
