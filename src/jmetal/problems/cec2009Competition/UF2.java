...
public class UF2 extends Problem {
	...
	/**
	 * Constructor. Creates a default instance of problem CEC2009_UF2 (30 decision
	 * variables)
	 * 
	 * @param solutionType
			...
			if (j % 2 == 0) {
				...
				yj = x[j - 1]
						- (0.3 * x[0] * x[0] * Math.cos(24 * Math.PI * x[0] + 4 * j * Math.PI / numberOfVariables_)
								+ 0.6 * x[0]) * Math.sin(6.0 * Math.PI * x[0] + j * Math.PI / numberOfVariables_);
				...
				yj = x[j - 1]
						- (0.3 * x[0] * x[0] * Math.cos(24 * Math.PI * x[0] + 4 * j * Math.PI / numberOfVariables_)
								+ 0.6 * x[0]) * Math.cos(6.0 * Math.PI * x[0] + j * Math.PI / numberOfVariables_);
