...
public class FogPlanningProblemFaisal extends Problem {
	...
	static final int[] foginventory = { 0, 20, 15, 10, 4 };
	static final int rent = 100;
	int BUDGET = 0;
	...
	public FogPlanningProblemFaisal() {
		// TODO Auto-generated constructor stub
	}

	public FogPlanningProblemFaisal(String solutionType) {
		this(solutionType, "90-10__ver1-try");
		// TODO Auto-generated constructor stub
	}

	public FogPlanningProblemFaisal(String solutionType, String inputname) {
		ri = new ReadInstance(inputname);
		numberOfVariables_ = ri.clients + (ri.fogs * 2);
		numberOfObjectives_ = ri.objectives_;
		numberOfConstraints_ = 3 * ri.fogs + 1;
		BUDGET = (int) (ri.fogs * (0.4 * 250000 + 100));
	...
	public void evaluate(Solution solution) throws JMException {
		...
		for (int j = 0; j < clients_tem; j++) {
			...
			if (dec == fogs_tem) {
				f[0] += ri.bandwidth_demand[j];
			} else {
				f[0] += 0.01 * ri.bandwidth_demand[j];
			}
		...
		for (int locationnum = 0; locationnum < fogs_tem; locationnum++) {
			...
			if (0 == deci) {
				solution.getDecisionVariables()[clients_tem + fogs_tem + locationnum].setValue(0.0);
				continue;
				...
				for (int j = 0; j < clients_tem; j++) {
					...
					if (locationnum == deci) {
						totalDemand += 0.01 * ri.bandwidth_demand[j] + 0.1;// KB
					}
				}
				...
				/*
				 * f[0]+= fogprice[cpuchoice]; //fog machine buying cost; f[0]+= (cpuchoice==
				 * 0)?0:rent; //location rental fee; f[0]+= linkprice[linkchoice];// link
	...
	public void evaluateConstraints(Solution solution) throws JMException {
		...
		for (int i = 0; i < fogs_tem; i++) {
			...
			constraint[i + fogs_tem] = fogmem[decii] - totalDemand;
		}
		...
		for (int i = 0; i < (fogs_tem); i++) {
			...
			constraint[i + 2 * fogs_tem] = (double) linkCp[deciii] - totalDemand;
		}
		...
		int cost = 0;
		for (int locationnum = 0; locationnum < fogs_tem; locationnum++) {
			int cpuchoice = (int) solution.getDecisionVariables()[clients_tem + locationnum].getValue();
			int linkchoice = (int) solution.getDecisionVariables()[clients_tem + fogs_tem + locationnum].getValue();

			cost += fogprice[cpuchoice]; // fog machine buying cost;
			// f[0]+= (cpuchoice== 0)?0:rent; //location rental fee;
			cost += linkprice[linkchoice];
		}
		constraint[3 * fogs_tem] = BUDGET - cost;
