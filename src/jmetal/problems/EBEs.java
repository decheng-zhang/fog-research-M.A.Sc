...
public class EBEs extends Problem {
	 ...
	 * 
	 * public void setMaxEvaluations(int maxEvaluations) { maxEvaluations_ =
	 * maxEvaluations; } // setNumberOfElement
	 * 
	 * public int getMaxEvaluations() { return maxEvaluations_; } //
	 * setNumberOfElement
	 */
	...
	double[] pi = new double[numberOfLibertyDegree_]; // variable auxiliar carga equivalente en nudo i referida al eje
														// local
	double[] pj = new double[numberOfLibertyDegree_]; // variable auciliar carga equivalente en nudo j referida al eje
														// local
	...
	public void EBEsInitialize() {
		...
		for (int gr = 0; gr < numberOfGroupElements_; gr++) {
			...
			if (Groups_[gr][SHAPE] == CIRCLE && Groups_[gr][VARIABLES] == 1) {
				...
				System.out.println(
						"Error in LIMITES LOWER/UPPER: transversal section not considerated for: " + gr + " group");
			} // end if
		} // gr
	...
	public void evaluate(Solution solution) throws JMException {
		...
		// NOT USED -----------------------------------
		/*
		 * double l=0; // longitud total de todos los elementos // total deflection of
		 * estructure fx[1]=0; for(int ba=0; ba<numberOfElements_; ba++){
		 * l+=Element_[ba][L_]; int ni = (int)Element_[ba][i_]; int nj =
	...
	public void evaluateConstraints(Solution solution) throws JMException {
		...
		for (int gr = 0; gr < numberOfGroupElements_; gr++) {
			...
			if (Groups_[gr][SHAPE] == CIRCLE && Groups_[gr][CONSTRAINT] == 0) {
				...
				/*
				 * if(Groups_[gr][RATIO_YZ]>=1.0) { if(x1/x2<Groups_[gr][RATIO_YZ]){ double swap
				 * = x1; x1=x2; x2=swap; x[var-4].setValue(x1); x[var-3].setValue(x2); } } else
				 * { if(x1/x2>Groups_[gr][RATIO_YZ]){ double swap = x1; x1=x2; x2=swap;
				 * x[var-4].setValue(x1); x[var-3].setValue(x2); } }
				 */
				...
				// // relaciÃ³n entre espesor de la placa y altura de lados
				constraint[con - 2] = Math.min(tb1, tb2);// -x1/x3+1.12*Math.sqrt(Groups_[gr][E_]/(Groups_[gr][STRESS]*2));
															// // relaciÃ³n entre espesor de la placa y altura de lados
				constraint[con - 1] = Math.min(ta1, ta2);// -x2/x4+1.12*Math.sqrt(Groups_[gr][E_]/(Groups_[gr][STRESS]*2.0));
															// // relaciÃ³n entre espesor de la placa y altura de lados
				...
				constraint[con - 3] = +ratio - Groups_[gr][RATIO_YZ] * 0.85; // /2.0 relaciÃ³n entre altura y base
																				// minima
				constraint[con - 2] = -x1 / x3 + 1.12 * Math.sqrt(Groups_[gr][E_] / (Groups_[gr][STRESS])); // relaciÃ³n
																											// entre
																											// espesor
																											// de la
																											// placa y
																											// altura de
																											// lados
				constraint[con - 1] = -x2 / x4 + 0.35 * Math.sqrt(Groups_[gr][E_] / (Groups_[gr][STRESS_CUT] * 2.0)); // relaciÃ³n
																														// entre
																														// espesor
																														// de
																														// la
				...
				x4 = x[var - 1].getValue(); // tickness along Z axis => tickness of heigth plate
				double ratio = x1 / x2;
				constraint[con - 4] = -ratio + Groups_[gr][RATIO_YZ] * 0.85; // /2.0 relaciÃ³n entre altura y base
																				// maxima
				constraint[con - 3] = +ratio - Groups_[gr][RATIO_YZ]; // relaciÃ³n entre altura y base minima
				constraint[con - 2] = -x1 / x3 + 0.35 * Math.sqrt(Groups_[gr][E_] / (Groups_[gr][STRESS_CUT] * 2.0)); // relaciÃ³n
																														// entre
																														// espesor
																														// de
																														// la
																														...
																														// de
																														// lados
				constraint[con - 1] = -x2 / x4 + 1.12 * Math.sqrt(Groups_[gr][E_] / (Groups_[gr][STRESS] * 2.0)); // relaciÃ³n
																													// entre
																													// espesor
																													// de
																													// la
				...
				constraint[con - 3] = +ratio - Groups_[gr][RATIO_YZ]; // relaciÃ³n entre altura y base minima
				constraint[con - 2] = -x1 / x3 + 1.12 * Math.sqrt(Groups_[gr][E_] / (Groups_[gr][STRESS_CUT] * 2.0)); // relaciÃ³n
																														// entre
																														// espesor
																														// de
																														// la
				...
				constraint[con - 4] = -ratio + Groups_[gr][RATIO_YZ]; // relaciÃ³n entre altura y base maxima
				constraint[con - 3] = +ratio - Groups_[gr][RATIO_YZ]; // relaciÃ³n entre altura y base minima
				constraint[con - 2] = -x1 / x3 + 1.12 * Math.sqrt(Groups_[gr][E_] / (Groups_[gr][STRESS] * 2.0)); // relaciÃ³n
																													// entre
																													// espesor
																													// de
																													// la
		...
		/*
		 * // LONGITUD TOTAL DE LA VIGA double l=0; for(int ba=0; ba<numberOfElements_;
		 * ba++){ l+=Element_[ba][2]; }
		 */
	...
	public void EBEsWeigthElement() throws JMException {
		...
		for (int el = 0; el < numberOfElements_; el++) {
			...
			// acumula cargas equivalentes en nudos en coordenadas LOCALES para la MISMA
			// BARRA E HIPÃ³TESIS
	...
	public void EBEsCalculus() throws JMException {
		...
		MatrixStiffness_ = new double[numberOfLibertyDegree_ * numberOfLibertyDegree_ * numberOfNodes_
		...
		// total equivalent load nodes in all structure witch load on elements and load
		// nodes
	...
	public void EBEsSteelingResults(int hi) {
		...
		for (int m = 0; m < numberOfLibertyDegree_ * numberOfLibertyDegree_ * numberOfNodes_
				* (elementsBetweenDiffGreat_ + 1); m++) {
	...
	public void EBEsMatrixGlobalFactory(int countIter) throws JMException {
		...
		for (int el = 0; el < numberOfElements_; el++) {
			...
			EBEsMatRot3DLaG(el);
			// formaciÃ³n y cÃ¡lculo de la matriz de rigidez de cada barra 3D en coordenadas
			// globales
			EBEsMat3DGij();
	...
	public void EBEsMatrixGlobalPenalization() {
		...
		for (int i = 0; i < numberOfNodesRestricts_; i++) {
			...
			if (w0 == '1') {
				MatrixStiffness_[matrixWidthBand_ * (numberOfLibertyDegree_ * no + aX_)] = 1.0E+35; // coacciÃ³n rÃ­gida
																									// en X
			} // coacciÃ³n rÃ­gida en X

			w0 = strCxyz.charAt(aY_); // sentido en Y
			if (w0 == '1') {
				MatrixStiffness_[matrixWidthBand_ * (numberOfLibertyDegree_ * no + aY_)] = 1.0E+35; // coacciÃ³n rÃ­gida
																									// en Y
			} // coacciÃ³n rÃ­gida en Y

			w0 = strCxyz.charAt(aZ_); // sentido en Z
			if (w0 == '1') {
				MatrixStiffness_[matrixWidthBand_ * (numberOfLibertyDegree_ * no + aZ_)] = 1.0E+35; // coacciÃ³n rÃ­gida
																									// en Z
			} // coacciÃ³n rÃ­gida en Z

			w0 = strCxyz.charAt(gX_); // rotaciÃ³n alrededor del eje X
			if (w0 == '1') {
				MatrixStiffness_[matrixWidthBand_ * (numberOfLibertyDegree_ * no + gX_)] = 1.0E+35; // coacciÃ³n rÃ­gida
																									// alrededor de X
			} // coacciÃ³n rÃ­gida de rotaciÃ³n en X

			w0 = strCxyz.charAt(gY_); // rotaciÃ³n alrededor del eje Y
			if (w0 == '1') {
				MatrixStiffness_[matrixWidthBand_ * (numberOfLibertyDegree_ * no + gY_)] = 1.0E+35; // coacciÃ³n rÃ­gida
																									// alrededor de Y
			} // coacciÃ³n rÃ­gida en Y

			w0 = strCxyz.charAt(gZ_); // rotaciÃ³n alrededor del eje Z
			if (w0 == '1') {
				MatrixStiffness_[matrixWidthBand_ * (numberOfLibertyDegree_ * no + gZ_)] = 1.0E+35; // coacciÃ³n rÃ­gida
																									// alrededor de Z
	...
	public void EBEsOverloadWeightElement() throws JMException {
		...
		for (int i = 0; i < numberOfWeigthsElements_; i++) {
			...
			// acumula cargas equivalentes en nudos en coordenadas GLOBALES la MISMA BARRA E
			// HIPÃ³TESIS
			// hipotesis asignada
	...
	public void EBEsWeightDistributedUniformly(int el, double[] LoadInElement_) throws JMException {
		...
		// Qi() cargas equivalentes aplicadas en el extremo i de la barra con sentido y
		// direcciÃ³n del sistema GLOBAL
		// Qj() cargas equivalentes aplicadas en el extremo j de la barra con sentido y
		// direcciÃ³n del sistema GLOBAL
		// pi() cargas equivalentes aplicadas en el extremo i con sentido y direcciÃ³n
		// del sistema LOCAL de cada barra
		// pj() cargas equivalentes aplicadas en el extremo j con sentido y direcciÃ³n
		// del sistema LOCAL de cada barra
		...
		// double lij = Math.sqrt(Math.pow((xj - xi), 2.0) + Math.pow((yj - yi), 2.0) +
		// Math.pow((zj - zi), 2.0));
	...
	public void EBEsMatRot3DLpSaL(int e) {
		// matriz de rotaciÃ³n 3D en ejes principales "yp,zp" de la secciÃ³n "S" a ejes
		// LOCALES "y,z"
		// cuando los ejes principales de la secciÃ³n estÃ¡n rotados un Ã¡ngulo Beta
		// respecto al sistema global

		int i, j;
		// cosenos directores de x local respecto al sistema global
		double lx; // cosenos directo respecto del eje local x (coincidente con el eje de la barra)
					// y el eje X Global
		double mx; // cosenos directo respecto del eje local x (coincidente con el eje de la barra)
					// y el eje Y Global
		double nx; // cosenos directo respecto del eje local x (coincidente con el eje de la barra)
					// y el eje Z Global
		// cosenos directores de y local respecto al sistema global
		double ly; // cosenos directo respecto del eje local y (coincidente con el eje de la barra)
					// y el eje X Global
		double my; // cosenos directo respecto del eje local y (coincidente con el eje de la barra)
					// y el eje Y Global
		double ny; // cosenos directo respecto del eje local y (coincidente con el eje de la barra)
					// y el eje Z Global
		// cosenos directores de z local respecto al sistema global
		double lz; // cosenos directo respecto del eje local z (coincidente con el eje de la barra)
					// y el eje X Global
		double mz; // cosenos directo respecto del eje local z (coincidente con el eje de la barra)
					// y el eje Y Global
		double nz; // cosenos directo respecto del eje local z (coincidente con el eje de la barra)
					// y el eje Z Global
		...
		// matriz de rotaciÃ³n de desplazamientos locales a ejes globales XYZ si los
	...
	public void EBEsMatRot3DLaG(int e) throws JMException {
		...
		double ly; // cosenos directo respecto del eje local y (coincidente con el eje de la barra)
					// y el eje X Global
		double my; // Single 'cosenos directo respecto del eje local y (coincidente con el eje de
					// la barra) y el eje Y Global
		double ny; // cosenos directo respecto del eje local y (coincidente con el eje de la barra)
					// y el eje Z Global
		// cosenos directores de z local respecto al sistema global
		double lz; // cosenos directo respecto del eje local z (coincidente con el eje de la barra)
	...
	public double[][][] EBEsStrainNode(double[][][] E) throws JMException {
		...
		for (int hi = 0; hi < numberOfWeigthHypothesis_; hi++) {
			for (int ba = 0; ba < numberOfElements_; ba++) {
				...
				// TENSIONES NORMALES en MPa (Mega Pascal = 1.000.000 Pa = 1.000 kN/m2, [1.0 Pa
				// = 1.0 N/mm2] )
	...
	public double BucklingOmega(double Nxx, double[] G, double[] B) throws JMException {
		...
		if (Nxx < 0.0 && G[AREA] > 0.0 && lBuckling) {
			...
			{
				System.out.println("Error in " + G[INDEX_] + " group, the material number " + G[TypeMaterial_]
						+ " is not implemented");
			} else if (G[TypeMaterial_] == 2) // F-24 steel 24 MN/m2
			{
				// Método Domke - Tabla Lamda0 - Lamda
				// double λ = -2.3443 + 1.07817 * λo - 0.0036181 * Math.pow(λo, 2) +
				// 0.000008209862 * Math.pow(λo, 3) - 0.00000001058458 * Math.pow(λo, 4) +
				// 0.000000000007147864 * Math.pow(λo, 5) - 0.00000000000000196410755 *
				// Math.pow(λo, 6);
				...
				double λ = λo;
				if (λ <= 150) {
					ω = 1.113 + 0.0070516 * λ - 0.000132108 * Math.pow(λ, 2.0) + 0.000002106132 * Math.pow(λ, 3.0)
							- 0.00000000397368332151 * Math.pow(λ, 4.0);
				} else
					ω = 25.0;
			}
			...
			else if (G[TypeMaterial_] == 20) // Wood hard an halt-hard
			{
				...
				double λ = λo;
				if (λ <= 150) {
					ω = 1.048 + 0.005524 * λ - 0.000101666 * Math.pow(λ, 2.0) + 0.00000301687 * Math.pow(λ, 3.0)
							- 0.000000004366246 * Math.pow(λ, 4.0);
				} else
					ω = 25.0;
			}
		}
			...
			for (int gr = 0; gr < numberOfGroupElements_; gr++) {
				...
				if (StrainMax_[gr][hi] != 0.0)
					StrainResidualMax_[hi] += Math
							.sqrt(Math.pow((StrainMax_[gr][hi] - Groups_[(int) Element_[gr][INDEX_]][STRESS]), 2.0));
				if (StrainMin_[gr][hi] != 0.0)
					StrainResidualMin_[hi] += Math.sqrt(
							Math.pow((-StrainMin_[gr][hi] + Groups_[(int) Element_[gr][INDEX_]][COMPRESSION]), 2.0));
				// residuos de tensiones tangenciales
				if (StrainCutMax_[gr][hi] != 0.0)
					StrainResidualCut_[hi] += Math.sqrt(
							Math.pow((StrainCutMax_[gr][hi] - Groups_[(int) Element_[gr][INDEX_]][STRESS_CUT]), 2.0));
			}
		}
	}
	...
	public void EBEsPrintArchTxtElements() throws JMException {
		try {
			...
			for (int gr = 0; gr < Groups_.length; gr++) {
				ps.printf("%4d %6.3f %6.3f %7.4f %7.4f %6.3f %6.3f %6.3f %6.3f %9.6f %9.6f %9.6f %9.6f %9.6f", gr,
						Groups_[gr][Y_], Groups_[gr][Z_], Groups_[gr][eY_], Groups_[gr][eZ_], Groups_[gr][uY_],
						Groups_[gr][dY_], Groups_[gr][lZ_], Groups_[gr][rZ_], Groups_[gr][Az_], Groups_[gr][Ay_],
						Groups_[gr][Iz_], Groups_[gr][Iy_], Groups_[gr][It_]);
				ps.println();
			} // Next ba
			ps.close();
		} catch (Exception ex) {
	...
	public void EBEsPrintArchTxtEfforts(int hi) throws JMException {
		try {
			...
			for (int ba = 0; ba < Element_.length; ba++) {
				int ni = (int) Element_[ba][i_];
				int nj = (int) Element_[ba][j_];
				ps.printf("Ei(%3d,%3d)=%10.3f  %10.3f  %10.3f  %10.3f  %10.3f  %10.3f", ba, ni, Efforti_[0][ba][hi],
						Efforti_[1][ba][hi], Efforti_[2][ba][hi], Efforti_[3][ba][hi], Efforti_[4][ba][hi],
						Efforti_[5][ba][hi]);
				ps.println();
				ps.printf("Ej(%3d,%3d)=%10.3f  %10.3f  %10.3f  %10.3f  %10.3f  %10.3f", ba, nj, Effortj_[0][ba][hi],
						Effortj_[1][ba][hi], Effortj_[2][ba][hi], Effortj_[3][ba][hi], Effortj_[4][ba][hi],
						Effortj_[5][ba][hi]);
				ps.println();
				ps.println();
			} // Next ba
		...
		try {
			...
			for (int hi = 0; hi < numberOfWeigthHypothesis_; hi++) {
				...
				for (int ba = 0; ba < Element_.length; ba++) {
					int ni = (int) Element_[ba][i_];
					int nj = (int) Element_[ba][j_];
					ps.printf("%4d  %4d  %10.3f  %10.3f  %10.3f", ba, ni, Straini_[STRAIN_TRACTION][ba][hi],
							Straini_[STRAIN_COMPRESS][ba][hi], Straini_[STRAIN_CUT][ba][hi]);
					ps.println();
					ps.printf("%4d  %4d  %10.3f  %10.3f  %10.3f", ba, nj, Strainj_[STRAIN_TRACTION][ba][hi],
							Strainj_[STRAIN_COMPRESS][ba][hi], Strainj_[STRAIN_CUT][ba][hi]);
					ps.println();
					ps.println();
				} // Next ba
				ps.close();
	...
	public String EBEsReadProblems() {
		...
		// en aquellos casos en los que se usen iteraciones en los algoritmos y no
		// evaluaciones hacer
		...
		try {
			...
			while (input.hasNext()) {
				...
				for (i = 0; i < numberOfElements_; i++) {
					...
					// coordenadas de los extremso de la barra
					/*
					 * if(Math.abs(Node_[ni][aX_])<= 0.000001) xi = 0.0; else xi=Node_[ni][aX_];
					 * 
					 * if(Math.abs(Node_[ni][aY_])<= 0.000001) yi= 0.0; else yi = Node_[ni][aY_];
					 * 
					 * if(Math.abs(Node_[ni][aZ_])<= 0.000001) zi = 0.0; else zi = Node_[ni][aZ_];
					 * 
					 * if(Math.abs(Node_[nj][aX_])<= 0.000001) xj = 0.0; else xj=Node_[nj][aX_];
					 * 
					 * if(Math.abs(Node_[nj][aY_])<= 0.000001) yj=0.0; else yj = Node_[nj][aY_];
					 * 
					...
					zj = Node_[nj][aZ_];
					Element_[i][L_] = Math
							.sqrt(Math.pow((xj - xi), 2.0) + Math.pow((yj - yi), 2.0) + Math.pow((zj - zi), 2.0));
