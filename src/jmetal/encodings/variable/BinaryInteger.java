...
public class BinaryInteger extends Binary {
	 ...
	 */
	public BinaryInteger(int numberOfBits, int lowerBound, int upperBound) {
		super(numberOfBits);
	 ...
	 *            Upper bound
	 */
	public BinaryInteger(BitSet bits, int nbBits, int lowerBound, int upperBound) {
		super(nbBits);
		bits_ = bits;
		lowerBound_ = lowerBound;
		upperBound_ = upperBound;
	 ...
	 */
	public BinaryInteger(BinaryInteger variable) {
		super(variable);
		 ...
		 * 
		 * bits_ = new BitSet(numberOfBits_); for (int i = 0; i < numberOfBits_; i++)
	...
	public void decode() {
		...
		value_ = (int) (value * ((double) upperBound_ - (double) lowerBound_) / (Math.pow(2.0, numberOfBits_) - 1.0)
				+ 0.5);
	...
	/**
	 * This implementation is efficient for binary string of length up to 24 bits,
	 * and for positive intervals.
	 * 
	...
	public void setValue(double value) throws JMException {
		if (numberOfBits_ <= 24 && lowerBound_ >= 0) {
			...
			if (value <= lowerBound_) {
				...
				/*
				 * int integerToCode = 0; double tmp = lowerBound_; double path = (upperBound_ -
				 * lowerBound_) / (Math.pow(2.0, numberOfBits_) - 1); while (tmp < value) { tmp
				 * += path; integerToCode++; }
				 */
			...
			this.bits_ = bitSet;
			// this.decode();
			if ((int) value != this.value_)
				throw new JMException("Binary casting error");
