...
public class SolutionSet implements Serializable {
	...
	public SolutionSet getSolutionsList_() {
		SolutionSet newList = new SolutionSet(capacity_);
		for (Solution aSolutionsList_ : solutionsList_) {
			// if (aSolutionsList_.getOverallConstraintViolation() == 0.0) {
			newList.add(aSolutionsList_);
			// }
		}
		return newList;
	}
	...
	public SolutionSet feasibleEnforcement() {
		SolutionSet newList = new SolutionSet(capacity_);
		for (Solution aSolutionsList_ : solutionsList_) {
			if (aSolutionsList_.getOverallConstraintViolation() == 0.0) {
				newList.add(aSolutionsList_);
	...
	public void printFeasibleFUNnVAR(String path) {
		try {
			FileOutputStream fos = new FileOutputStream(path);
			OutputStreamWriter osw = new OutputStreamWriter(fos);
			BufferedWriter bw = new BufferedWriter(osw);
